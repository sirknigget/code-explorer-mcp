...
def get_project_structure(
    ...
    *,
    ...
    normalized_subfolder = (
        ...
        else project_relative_path(project_root, request.subfolder)
    )
    start_path = (
        project_root
        if normalized_subfolder is None
        else project_root / normalized_subfolder
    )
    ...
    for current_root, dirnames, filenames in start_path.walk(top_down=True):
        ...
        for dirname in sorted(dirnames):
            if dirname in COMMON_IGNORED_DIRECTORIES:
                continue
            relative_directory = project_relative_path(
                project_root, current_root / dirname
            )
        ...
        for filename in sorted(filenames):
            relative_file = project_relative_path(project_root, current_root / filename)
            if gitignore_matcher.matches_file(relative_file):
                continue
            if patterns and not any(
                fnmatchcase(relative_file, pattern) for pattern in patterns
            ):
                continue
            matched_paths.append(relative_file)
